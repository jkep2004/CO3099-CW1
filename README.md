...
The program must be in a file called WannaCry.java. It is run by the command java WannaCry(again, of course no real ransomware is started by users like that, but it is only a prototype.)
...
Obviously, the gang members don't want to use their personal RSA keypairs for encryption as they don't want to be identified. Instead, they generated a separate master RSA keypair. Here is the public key of that keypair: ``
...
An example of the client execution may look like this: (...quietly encrypting without any message on screen...)

`Dear User! Please note that your files have now been encrypted. To recover your files we ask you to follow the instructions in the website below to arrange a small payment: https://...`

## The decryption client

The program must be in a file called Decryptor.java. The victims run your program with the command: `java Decryptor host port userid`
...
Finally, if the server verified the signature, the client will receive the decrypted AES key, and it should use it to decrypt the contents of the test.txt.cry file and save it back to test.txt. An example of the client execution may look like this:

[successful invocation]
`Dear customer, thank you for purchasing this software. We are here to help you recover your files from this horrible attack. Trying to decrypt files... Success! Your files have now been recovered!`

[unsuccessful invocation]
`Dear customer, thank you for purchasing this software. We are here to help you recover your files from this horrible attack. Trying to decrypt files... Unfortunately we cannot verify your identity. Please try again, making sure that you have the correct signaturekey in place and have entered the correct userid.`
